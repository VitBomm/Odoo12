...
class Add_Invoice_Wizard(models.TransientModel):
    ...
    # @api.onchange('chiphi_id')
    # @api.multi
    # def check_selected_partner(self):
    #     if self.chiphi_id == False:
    #         raise exceptions.ValidationError('Vui Lòng Chọn Đối Tác rồi chọn chi phí')
    ...
    def on_change_partner(self):
        if self.user_id:
            #'state','=','approve'
            # templist = self.env['hhd.cost.recovery'].search([('partner_id.id','=',self.partner.id)])
